...
from app.database.models.custom_template import CustomTemplate, CustomTemplateCreate, CustomTemplateUpdate
...
class CustomTemplateRepository(BaseRepository):
    ...
    async def create_template(self, template_data: CustomTemplateCreate, user_id: str) -> str:
        ...
        try:
            ...
            result = await self.db[self.collection_name].insert_one(template.model_dump())
    ...
    async def get_template(self, template_id: str) -> CustomTemplate | None:
        ...
        try:
            doc = await self.db[self.collection_name].find_one({"_id": ObjectId(template_id)})
    ...
    async def get_user_templates(self, user_id: str, limit: int = 50) -> List[CustomTemplate]:
    ...
    async def update_template(self, template_id: str, update_data: CustomTemplateUpdate) -> bool:
    ...
    async def search_templates(self, query: str, limit: int = 50) -> List[CustomTemplate]:
        ...
        try:
            ...
            cursor = self.db[self.collection_name].find(search_filter).sort("rating", -1).limit(limit)
            docs = await cursor.to_list(length=limit)
