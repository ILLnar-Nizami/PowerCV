...
class CVValidator:
    ...
    def validate_optimization(original: str, optimized: str) -> Dict:
        ...
        # Check LinkedIn
        if (
            orig_contact.get("linkedin")
            and orig_contact["linkedin"].lower() not in optimized.lower()
        ):
            errors.append(f"Missing LinkedIn: {orig_contact['linkedin']}")

        # Check GitHub
        if (
            orig_contact.get("github")
            and orig_contact["github"].lower() not in optimized.lower()
        ):
        ...
        for skill in hallucination_skills:
            if skill in optimized.lower() and skill not in original.lower():
                warnings.append(f"Possible hallucination: '{skill}' not in original")
        ...
        orig_has_certs = any(keyword in original.lower() for keyword in cert_keywords)
        opt_has_certs = any(keyword in optimized.lower() for keyword in cert_keywords)
        ...
        if "address" in original.lower() or "netherlands" in original.lower():
            if (
                "netherlands" in original.lower()
                and "netherlands" not in optimized.lower()
            ):
    ...
    def validate_required_fields(cv_text: str) -> Dict:
        ...
        return {
            ...
            "present_fields": [
                field for field, present in required_fields.items() if present
            ],
        }
