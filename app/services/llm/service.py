...
class UniversalLLMService:
    ...
    def __init__(self, provider: Optional[BaseLLMProvider] = None, **config_overrides):
        ...
        logger.info(
            f"Initialized LLM service with {self.provider.provider_type.value} provider"
        )
    ...
    async def extract_json(
        ...
        **kwargs,
        ...
        response = await self.complete(
            prompt=full_prompt, temperature=temperature, **kwargs
        )
    ...
    async def analyze_resume_match(
        self, resume_text: str, job_description: str, temperature: float = 0.1, **kwargs
    ) -> Dict[str, Any]:
...
Provide the optimized version of this section. Make it more impactful while maintaining accuracy."""
    ...
    def _normalize_messages(
        self, messages: Union[List[LLMMessage], List[Dict[str, str]], List[str]]
    ) -> List[LLMMessage]:
...
async def analyze_resume_match(
    resume_text: str, job_description: str, **kwargs
) -> Dict[str, Any]:
