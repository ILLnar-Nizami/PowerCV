...
class UniversalLLM:
    ...
    def __init__(self, **config_overrides):
        ...
        logger.info(
            f"Universal LLM initialized with {self.provider.provider_type.value}"
        )
    ...
    async def chat(self, messages: Union[str, List[Dict[str, str]]], **kwargs) -> str:
        ...
        response = await self.provider.chat_completion(
            messages=[self.provider._convert_message(msg) for msg in messages], **kwargs
        )
    ...
    async def analyze_match(
        self, resume_text: str, job_description: str, **kwargs
    ) -> Dict[str, Any]:
