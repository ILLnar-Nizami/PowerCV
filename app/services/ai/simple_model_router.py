...
class SimpleModelRouter:
    ...
    def __init__(self):
        if not hasattr(self, "initialized"):
            ...
            # Configuration
            provider = (
                os.getenv("API_TYPE") or os.getenv("LLM_PROVIDER") or ""
            ).lower()
            ...
            if cerebras_key and not force_local:
                ...
                default_balanced = os.getenv(
                    "CEREBRAS_MODEL_NAME", settings.API_MODEL_NAME
                )
            else:
    ...
    def _execute_task(
        self, model_name: str, task_name: str, **kwargs
    ) -> Dict[str, Any]:
        """Execute a specific task using the model."""
        temperature = (
            0.0 if task_name in ["extract_keywords", "parse_resume_structure"] else 0.6
        )
...
Current skills: {existing_skills}"""
            ...
            text = kwargs.get("text", "")
            return (
                f"Optimize this text for psychological impact and persuasion:\n\n{text}"
            )
        ...
        try:
            ...
            if task_name in [
                ...
                if json_match:
                    ...
                    if task_name == "extract_keywords" and isinstance(
                        parsed_data, list
                    ):
