...
class ErrorHandler:
    ...
    @staticmethod
    def log_error(
        error: Exception, context: Dict[str, Any] = None, include_traceback: bool = True
    ) -> Dict[str, Any]:
    ...
    def handle_ai_api_error(
        error: Exception, provider: str, operation: str, context: Dict[str, Any] = None
    ) -> HTTPException:
        ...
        if "timeout" in str(error).lower():
            ...
            user_message = (
                f"Unable to connect to {provider} service. Please try again later."
            )
        elif (
            ...
            user_message = (
                f"An error occurred with the {provider} service. Please try again."
            )
...
class ValidationError(DetailedError):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            message=message, error_code="VALIDATION_ERROR", context=context, **kwargs
        )
