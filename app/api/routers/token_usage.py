...
async def get_token_usage_summary(
    ...
    feature: Optional[str] = Query(
        None, description="Filter by specific feature (optional)"
    ),
    user_id: Optional[str] = Query(
        None, description="Filter by specific user (optional)"
    ),
) -> TokenUsageSummary:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error generating token usage summary: {str(e)}"
        )
...
async def export_token_usage_data(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error exporting token usage data: {str(e)}"
        )
...
async def get_model_pricing() -> Dict:
    ...
    return {"model_pricing": MODEL_PRICING, "note": "Prices are in USD per 1000 tokens"}
