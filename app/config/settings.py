...
class Settings(BaseSettings):
    ...
    app_name: str = Field(
        default="PowerCV", validation_alias=AliasChoices("APP_NAME"))
    version: str = Field(
    ...
    reload: bool = Field(
        default=False, validation_alias=AliasChoices("RELOAD"))
    ...
    algorithm: str = Field(default="HS256", validation_alias=AliasChoices(
        "ALGORITHM", "JWT_ALGORITHM"))
    access_token_expire_minutes: int = Field(
        default=30, validation_alias=AliasChoices("ACCESS_TOKEN_EXPIRE_MINUTES", "JWT_EXPIRATION_HOURS")
    )
    ...
    redis_port: int = Field(
        default=6379, validation_alias=AliasChoices("REDIS_PORT"))
    ...
    openai_api_base: str = Field(
        default="https://api.openai.com/v1", validation_alias=AliasChoices("OPENAI_API_BASE"))
    ...
    rate_limit_requests_per_minute: int = Field(default=100, validation_alias=AliasChoices(
        "RATE_LIMIT_REQUESTS_PER_MINUTE", "RATE_LIMIT_PER_MINUTE"))
    rate_limit_requests_per_hour: int = Field(default=1000, validation_alias=AliasChoices(
        "RATE_LIMIT_REQUESTS_PER_HOUR", "RATE_LIMIT_PER_HOUR"))

    # File upload settings
    max_file_size: int = Field(
        # 10MB
        default=10 * 1024 * 1024, validation_alias=AliasChoices("MAX_FILE_SIZE"))
    allowed_file_types: List[str] = Field(
        default=["pdf", "doc", "docx", "txt"], validation_alias=AliasChoices("ALLOWED_FILE_TYPES"))
