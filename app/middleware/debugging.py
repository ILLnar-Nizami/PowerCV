...
class DebuggingMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app, enable_debug: bool = False):
        super().__init__(app)
        self.enable_debug = (
            enable_debug and os.getenv("ENVIRONMENT", "development") == "development"
        )
...
class PerformanceMonitoringMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(
        ...
        if processing_time > self.slow_request_threshold_ms:
            logger.warning(
                f"Slow request detected: {request.method} {request.url.path} took {processing_time:.2f}ms",
                extra={
                    "method": request.method,
                    "path": request.url.path,
                    "processing_time_ms": processing_time,
...
class RequestLoggingMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(
        ...
        processing_time = (time.time() - start_time) * 1000
        log_data.update(
            {"status_code": response.status_code, "processing_time_ms": processing_time}
        )
...
def setup_debugging_middleware(app, enable_debug: bool = False):
    ...
    app.add_middleware(
        PerformanceMonitoringMiddleware, slow_request_threshold_ms=1000.0
    )
