...
class RetryContext:
    ...
    async def __aexit__(self, exc_type, exc_val, exc_tb):
        ...
        # Check if exception is retryable
        if any(isinstance(exc_val, exc_type) for exc_type in self.config.non_retryable_exceptions):
            return False  # Don't retry, non-retryable exception
        
        if not any(isinstance(exc_val, exc_type) for exc_type in self.config.retryable_exceptions):
