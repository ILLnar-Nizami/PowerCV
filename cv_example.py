...
def example_1_analyze_cv():
    ...
    try:
        ...
        for kw in result["keyword_analysis"]["matched_keywords"][:5]:
            print(
                f"  • {kw['keyword']} (JD: {kw['jd_mentions']}x, CV: {kw['cv_mentions']}x)"
            )

        print("\n  Missing Critical Keywords:")
        for kw in result["keyword_analysis"]["missing_critical"][:5]:
            print(
                f"  • {kw['keyword']} ({kw['category']}) - Priority: {kw['priority']}"
            )
...
def example_5_full_workflow():
    ...
    try:
        ...
        result = orchestrator.optimize_cv_for_job(
            cv_text=SAMPLE_CV, jd_text=SAMPLE_JD, generate_cover_letter=True
        )
...
def example_6_batch_testing():
        ...
        try:
            ...
            results[role] = {
                ...
                "missing_keywords": len(
                    analysis["keyword_analysis"]["missing_critical"]
                ),
            }
    ...
    sorted_results = sorted(
        results.items(), key=lambda x: x[1]["ats_score"], reverse=True
    )
