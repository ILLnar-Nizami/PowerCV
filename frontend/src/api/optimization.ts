...
async function getCvText(request: OptimizationRequest): Promise<string> {
	...
	if (request.sourceType === "master_cv" && request.sourceId) {
		const { data } = await apiClient.get(
			`/v1/resumes/master-cv/${request.sourceId}`,
		);
...
function transformAnalysisResponse(
	...
	const recommendations: Recommendation[] = rawRecommendations.map(
		(rec, index) => {
...
function transformOptimizationResponse(
	data: BackendOptimizationResponse,
): OptimizationResult {
	return {
		resumeId: data.resume_id || data.resumeId,
		improvements:
			data.improvements ||
			(data.optimized_resume ? ["Resume optimized successfully"] : []),
