...
import type { DashboardFilters, Resume } from "@/types/resume";
import { apiClient } from "./client";

export const resumesAPI = {
	getResumes: async (_filters: DashboardFilters, page = 1, pageSize = 20) => {
		const { data } = await apiClient.get<PaginatedResponse<Resume>>(
	...
	downloadOriginalResume: async (id: string) => {
		const { data } = await apiClient.get<Blob>(
			`/v1/resumes/${id}/download-original`,
			{
				responseType: "blob",
			},
		);
