...
export function OptimizePage() {
	...
	const { request, setRequest, setCurrentStep, step, setAnalysis } =
	...
	const { data: masterCVs = [] } = useQuery({
		queryKey: ["master-cvs"],
		queryFn: async () => {
			const { data } = await apiClient.get<MasterCVFromAPI[]>(
	...
	const handleAnalyze = async () => {
		setIsAnalyzing(true);
		try {
			const analysisResult = await optimizationAPI.analyze(
				request as OptimizationRequest,
			);
			...
			<div className="flex items-center gap-4">
				...
				<div>
					...
					<p className="text-muted-foreground">
						Create an ATS-optimized resume for your job application
					</p>
				</div>
			</div>
			...
			<Card>
				<CardHeader>
					<CardTitle className="flex items-center gap-2">
						...
						Step {step}:{" "}
						{["Select Source", "Job Details", "Template", "Review"][step - 1]}
					</CardTitle>
				</CardHeader>
					...
					{step === 1 && (
						<div className="space-y-4">
							...
							{request.sourceType === "upload" ? (
								<FileUpload
									onFileSelect={(file) =>
										setRequest({ ...request, uploadedFile: file || undefined })
									}
								...
								<div>
									...
									<Select
										onValueChange={(value) =>
											setRequest({ ...request, sourceId: value })
										}
									>
										<SelectTrigger>
										...
										<SelectContent>
											{masterCVs.length === 0 ? (
												<SelectItem value="no-cvs" disabled>
													No Master CVs found - upload one first
												</SelectItem>
											) : (
												masterCVs.map((cv) => (
						...
						<div className="space-y-4">
							...
							<div>
								...
								<Textarea
									...
									onChange={(e) =>
										setRequest({ ...request, jobDescription: e.target.value })
									}
						...
						<div className="space-y-6">
							...
							<div className="flex items-center justify-between p-4 border rounded-lg">
								<div className="space-y-0.5">
									<Label htmlFor="cover-letter-toggle" className="text-base">
										Generate Cover Letter
									</Label>
									<p className="text-sm text-muted-foreground">
										Create a tailored cover letter alongside your optimized
										resume
									</p>
								</div>
								<Switch
									...
									onCheckedChange={(checked) =>
										setRequest({ ...request, generateCoverLetter: checked })
									}
								/>
							</div>
						</div>
						...
						<div className="space-y-4">
							...
							<div className="bg-muted p-4 rounded-lg space-y-2">
								...
								<div>
									...
									{request.generateCoverLetter ? "Yes" : "No"}
								</div>
							</div>
						</div>
					)}
